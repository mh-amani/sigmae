...
class SigmaeLitModuleImageToTextOnVQVAETokens(SigmaeLitModuleImageToTextBase):
    ...
    def _initialize_models(self, models_config: Dict[str, torch.nn.Module]) -> None:
        ...
        self.vqvae = CustomVQGanVAE()
        self.vqvae.load_state_dict(torch.load("/dlabscratch1/amani/sigmae/data/vqgan_cifar10/vqgan_cifar10.pth"))
        self.vqvae.to(self.device)
        self.vqvae.train()
        self.training_modes = ['xzx', 'zxz', 'xz', 'zx']
    ...
    def _initialize_metrics(self) -> None:
        ...
        for split in ['learn', 'val', 'test']: # you can't use 'train' as a key ... it's a reserved word
            ...
            for training_mode in self.training_modes:
                ...
                self.accuracies[split].update({training_mode: Accuracy(task="multiclass", num_classes=1024 if training_mode.endswith('z') else self.hparams.model_params.x_vocab_size)})
                # else:
                #     self.accuracies[split].update({training_mode: StructuralSimilarityIndexMeasure()})
            
    def setup(self, stage: str):
        y_max_length = self.hparams.model_params.max_x_length
        y_vocab_size = self.hparams.model_params.x_vocab_size
        if torch.all(self.trainer.datamodule.data_train.random_sequence == -1):
            self.trainer.datamodule.data_train.random_sequence = self.trainer.datamodule.data_train._create_random_batch_of_tokens(len(self.trainer.datamodule.data_train), vocab_size=y_vocab_size, max_num_tokens=y_max_length)

    
    def forward(self, x, z, y=None, data_type=None, stage='learn', training_mode=None, output_image=False) -> torch.Tensor:
        outputs = {}
        labels = {}
        
        if output_image or stage=='val':
            training_mode = 'zxz'
        else:
            if data_type==[1, 1]:
                training_mode = 'xz_zx'
        ...
        if training_mode == 'zxz':
            ...
            codes = x.reshape(x.shape[0], -1)
            bos_token_id = self.auto_reg_wrapped_model_xz.control_token_ids['output_bos_token_id']
            prepended_codes = torch.cat([bos_token_id*torch.ones((codes.shape[0], 1), dtype=codes.dtype, device=codes.device), codes], dim=1)
            zxz_outputs = self.symbolic_autoencoder_wrapper_zxz(x_ids=prepended_codes, 
        ...
        elif training_mode=='xz_zx':
            codes = x.reshape(x.shape[0], -1)
            bos_token_id = self.auto_reg_wrapped_model_xz.control_token_ids['output_bos_token_id']
            prepended_codes = torch.cat([bos_token_id*torch.ones((codes.shape[0], 1), dtype=codes.dtype, device=codes.device), codes], dim=1)
            
            xz_outputs = self.auto_reg_wrapped_model_xz(input_ids=y, output_ids=codes, teacher_force_output=True)
            outputs['xz'] = xz_outputs
            labels['xz'] = codes
            
            zx_outputs = self.auto_reg_wrapped_model_zx(input_ids=codes, output_ids=y, teacher_force_output=True)
    ...
    def model_step(self, batch, batch_idx=-1, stage='learn', log_things=True):
        ...
        data_type = torch.all(data_type, dim=0).int().tolist()

        total_loss = 0
        output_image = batch_idx == 0

        if stage == 'val':
            # Loop over each training mode during validation
            # for training_mode in self.training_modes:
            outputs, labels = self.forward(x, z, y, data_type, stage=stage, output_image=output_image)
