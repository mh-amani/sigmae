...
class AutoRegWrapperSkipConn(AbstractAutoRegWrapper):
    """
    ...
    def __init__(
        ...
        assert self.config['skip_epsilon'] >= 0 and self.config['skip_epsilon'] <= 1, "skip_epsilon should be between 0 and 1"
        self.skip_epsilon = self.config['skip_epsilon']
    ...
    def teacher_forced_model_forward(
        ...
        raise NotImplementedError("Teacher forcing is not implemented for this model")
    ...
    def post_one_step_seq_forward(
        ...
        **kwargs,
        ...
        vector_encoder = self.skip_epsilon * current_output['vector_encoder'] + (1 - self.skip_epsilon) * output_embeds_encs[:, -1].unsqueeze(1)
        vector_decoder = self.skip_epsilon * current_output['vector_decoder'] + (1 - self.skip_epsilon) * output_embeds_decs[:, -1].unsqueeze(1)
        
        output_embeds_encs = torch.cat((output_embeds_encs, vector_encoder), dim=1)
        output_embeds_decs = torch.cat((output_embeds_decs, vector_decoder), dim=1)
